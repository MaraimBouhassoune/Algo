...
```
❌ ERREUR : Le fichier transactions_immobilieres.csv n'existe pas !
```

→ Vérifier que le fichier CSV est dans le même dossier

**Données invalides**

```
⚠️ Des erreurs ont été détectées, mais les tests continuent...
```

→ Utiliser `valider_donnees()` pour identifier les problèmes

**Performance anormale**

```
❌ Ratio comp/théorie suspect: 15.2
```

→ Vérifier l'implémentation de l'algorithme

### Mode debug

```python
# Activer les traces détaillées
from utilitaires import afficher_statistiques_dataset
afficher_statistiques_dataset(biens)
```

## 🏆 Critères d'évaluation

- **Implémentation correcte** (40 points)
- **Comptage précis des opérations** (20 points)
- **Mesures de performance** (20 points)
- **Analyse des résultats** (20 points)
...
- 🔧 **Extensible** : Facile d'ajouter de nouveaux algorithmes
- 🧪 **Testable** : Suite de tests automatisés
- 📊 **Analytique** : Rapports détaillés des performances

---

**📧 Questions ?** Consulter les commentaires dans le code ou utiliser les tests de validation.
